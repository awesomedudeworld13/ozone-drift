...
SHARED FILE -- byte-identical to ``phishdrift/evaluate.py`` in the sibling
phish-drift repository (sha256 40da0e59d87ab3f63b7acc54cbec771509002de6...).

It is duplicated rather than imported because the two repositories are
independent and neither should need the other installed to run. That trade has
a cost worth naming: **if these copies drift, the cross-domain comparison
silently stops being a comparison.** The whole multi-domain claim -- that the
kind of distribution shift predicts the size of the benchmark-to-operational gap
-- depends on every domain's TSS being the same quantity computed the same way.

``tests/test_shared.py`` pins the hash of this file so a local edit is caught.
It cannot detect upstream drift; when a third domain appears, extract this into
a package all three depend on rather than making a third copy.

Only the docstring above differs from the original. Do not edit anything below
it in this repository -- change it in phish-drift and re-copy.
